...
function searchInOutputContent(output: Output, searchTerm: string): boolean {
  try {
    ...
    if (output.type === 'objection' && Array.isArray(output.output)) {
      ...
      return output.output.some((rebuttal: ObjectionRebuttal) =>
      ...
      if (Array.isArray(messages)) {
        return messages.some((msg: MessageOutput) =>
      ...
      if (framework.discoveryQuestions && Array.isArray(framework.discoveryQuestions)) {
        const questionMatch = framework.discoveryQuestions.some((q: { question?: string; category?: string }) =>
    ...
    if (output.type === 'message') {
      ...
      return {
        ...
        metadata: Array.isArray(output.output)
          ? `${output.output.map((v: ObjectionRebuttal) => v.variant).join(' • ')}`
      ...
      return (
        <div className="space-y-4">
          ...
          {Array.isArray(output.output) ? (
            // New format with multiple variants
            <div className="space-y-6">
              {output.output.map((variant: ObjectionRebuttal, index: number) => (
                <div key={index} className="border-l-4 border-blue-500 pl-4 py-2">
